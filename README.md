...
The code is tested and runs but it does not do much except for showing the board id and the values of the analog input and knobs. The digital input is also scanned for triggers with a very simple PIO program and a info message is shown when the interrupt is raised. If things go according to plan (which they never but even so) more functionality will be added in the forseeable future.
