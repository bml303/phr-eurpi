...
use tasks::{
    ...
    I2C_BUS_FREQUENCY_400_KBIT, core0_task, core1_task, osc_task_consolidated, osc_task_dac,
    osc_task_generate, pio_task_sm0, pio_task_sm1, pio_task_sm2, pio_task_sm2_irq1,
    pio_task_sm2_irq2, setup_pio_task_sm0, setup_pio_task_sm1, setup_pio_task_sm2,
};
use utils::Debouncer;
...
fn main() -> ! {
    ...
    let i2c0 = i2c::I2c::new_blocking(p.I2C0, scl_0, sda_0, i2c0_config);
    ...
    let dma_out_ref = dma::Channel::new(p.DMA_CH0, IrqsPioDma);
    ...
    executor0.run(|spawner| {
        ...
        spawner.spawn(unwrap!(pio_task_sm2_irq2(irq2)));
        spawner.spawn(unwrap!(pio_task_sm2(sm2, Some(dma_out_ref))));
    });
